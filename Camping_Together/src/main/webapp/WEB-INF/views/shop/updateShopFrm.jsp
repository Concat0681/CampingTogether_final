...
<link rel="stylesheet" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.css" /><script src="/summernote/summernote-lite.js"></script>
...
<body>
	...
	<div class="page-wrap">
		...
		<form action="/updateShop.do" method="post" enctype="multipart/form-data">
			...
			<div class="input-group mb-3">
				...
				<input type="hidden" name="shopCategory" value="${shop.shopCategory}">	
			</div>
			<div class="input-group mb-3">
				<label for="shopTitle" class="input-group-text">제목</label>
				<input id="shopTitle"  class="form-control" type="text" name="shopTitle" value="${shop.shopTitle }" required>
			</div> 
			<div class="input-group mb-3">
				<label for="shopPhotoList" class="input-group-text">상품이미지</label>
				<input id="shopPhotoList" class="form-control" type="file" name="shopFileList" onchange="uploadPhoto(this);" multiple>
			</div>
			<div id="img-viewer">
				<c:forEach items="${shop.shopPhotoList }" var="p">
					<div class="img-box">
						<img src="resources/upload/shop/${p.filepath }">
						<button type="button" class="btn1" onclick="deleteOldPhoto(this, '${p.filepath}')">삭제</button>
					</div>
				</c:forEach>
			</div>
			<div class="shop-price-wrap">
				<div class="input-group mb-3">
					...
					<input id="shopPrice" type="text" class="number-input form-control" name="shopPrice" value="${shop.shopPrice }" required>
					<div class="check-result input-group-text"></div>
				</div>
				<div class="input-group mb-3">
					<label for="delivaryPrice" class="input-group-text">배송비</label>
					<input id="delivaryPrice" type="text" class="number-input form-control" name="delivaryPrice" value="${shop.delivaryPrice }" required>
					<div class="check-result input-group-text"></div>
				</div>
				<div class="input-group mb-3">
					<label for="maxCount" class="input-group-text">수량</label>
					<input id="maxCount" type="text" class="number-input form-control" name="maxCount" value="${shop.maxCount }" required>
					<div class="check-result input-group-text"></div>
				</div>
			</div>
			<div class="shop-content-wrap">
				<textarea id="shop-content" name="shopContent">${shop.shopContent}</textarea>
			</div>
	...
	function deleteOldPhoto(obj, filepath){
		destroySlick();
		$(obj).parent().parent().after("<input type='hidden' name='delPhotoList' value='"+filepath+"'>");
		$(obj).parent().remove();
		setTimeout(function () {
			applySlick()
        }, 10);
	...
	$(function(){
		$(".shop-category-btn").eq($("input[name=shopCategory]").val()).click();
		applySlick()
	})
	</script>
</body>
